...
from basenet import *
import os
from HybridSN import SSSE, SSSE_test
...
# mask settings
parser.add_argument('--A_mask_ratio', default=0.7, type=float)
...
parser.add_argument('--spectral_A_block_size', default=13, type=int)
parser.add_argument('--spatial_A_block_size', default=2, type=int)
parser.add_argument('--B_mask_ratio', default=0.7, type=float)
...
parser.add_argument('--spectral_B_block_size', default=13, type=int)
...
parser.add_argument('--halfwidth', default=1, type=int)
# mcc loss
parser.add_argument('--temperature', default=7.0,
...
seeds = [1558,1376,1313,1851,1717,1836,1409,1460,1484,1617]
...
def get_sample_data(Sample_data, Sample_label, HalfWidth, num_per_class):
    ...
    for i in range(m):
        ...
        num = len(indices)
        if num < num_per_class:
            train[i] = indices[:num_per_class]
            train[i] += indices[:num_per_class-num]
        else:
            train[i] = indices[:num_per_class]
...
for iDataSet in range(nDataSet):
    data_path_s = 'data/HyRANK/Dioni.mat'
    label_path_s = 'data/HyRANK/Dioni_gt_out68.mat'
    data_path_t = 'data/HyRANK/Loukia.mat'
    label_path_t = 'data//HyRANK/Loukia_gt_out68.mat'

    data_s, label_s = utils.load_data_hyrank(data_path_s, label_path_s)
    data_t, label_t = utils.load_data_hyrank(data_path_t, label_path_t)
    print('#######################idataset######################## ', iDataSet)
    utils.seed_everything(seeds[iDataSet],use_deterministic=True)

    trainX, trainY = get_sample_data(data_s, label_s, HalfWidth, 180)
    ...
    ##########################################################################################################
    G = SSSE_test(input_channels=N_BANDS,
    ...
    for ep in range(1, num_epoch + 1):
        ...
        if ep % num_epoch == 0:
            ...
            value, pred1_list, pred2_list, label_list, acc1, acc2, outputdata_target, target_label, predict, labels = test(test_loader)
            ...
            if acc1 >= best_test_acc:
                # torch.save(G, "checkpoints/hyrank/G.pt")
                # torch.save(F1, "checkpoints/hyrank/F1.pt")
                # torch.save(F2, "checkpoints/hyrank/F2.pt")
                best_test_acc = acc1

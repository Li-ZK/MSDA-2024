...
parser.add_argument('--spectral_A_block_size', default=15, type=int)
parser.add_argument('--spatial_A_block_size', default=3, type=int)
...
parser.add_argument('--spectral_B_block_size', default=15, type=int)
parser.add_argument('--spatial_B_block_size', default=3, type=int)
...
for iDataSet in range(nDataSet):
    data_path_s = 'data/Houston2013/data.mat'
    label_path_s = 'data/Houston2013/mask_train.mat'
    data_path_t = 'data//Houston2013/data.mat'
    label_path_t = 'data//Houston2013/mask_test.mat'

    data, label_train, label_test = utils.load_data_houston13(data_path_s, label_path_s,label_path_t)
    # data_t, label_t = utils.load_data_houston13(data_path_t, label_path_t)
    print('#######################idataset######################## ', iDataSet)
    utils.seed_everything(seeds[iDataSet], use_deterministic=False)

    _,trainX, trainY,_,_,_,_ = utils.get_all_data(data, label_train, HalfWidth)

    testID, testX, testY, Gr, RandPerm, Row, Column = utils.get_all_data(data, label_test, HalfWidth)

...
class ResClassifier(nn.Module):
    def __init__(self, class_num, extract=False, training=False, dropout_p=0.5):
        ...
        self.fc11 = nn.Linear(76, class_num)
...
class New_ResClassifier(nn.Module):
    def __init__(self, num_classes=12, num_unit=2048, prob=0.2, middle=1000,
                 extract=False, training=False, dropout_p=0.5):
        super(New_ResClassifier, self).__init__()
        layers1 = []
    ...
    def forward(self, x, reverse=False):
        fc13_emb = self.classifier1(x)
        logit = self.fc13(fc13_emb)
        # if self.training:
        #     fc1_emb.mul_(math.sqrt(1 - self.dropout_p))
        # fc2_emb = self.fc2(fc1_emb)
        # if self.training:
        ...
        # logit = self.fc11(fc1_emb)
        if self.training:
            return logit, fc13_emb

        if self.extract:
            return fc13_emb, logit
        return logit


class LCCDA_ResClassifier(nn.Module):
    def __init__(self, class_num, extract=False, training=False, dropout_p=0.5):
        super(LCCDA_ResClassifier, self).__init__()
        self.fc0 = nn.Sequential(
            nn.Linear(288, 152),
            nn.BatchNorm1d(152, affine=True),
            nn.ReLU(inplace=True),
            nn.Dropout(p=dropout_p)
        )
        self.fc1 = nn.Sequential(
...
class ResClassifier1(nn.Module):
    def __init__(self, num_classes, num_layer=2, num_unit=152,prob=0.5,middle=76):
        super(ResClassifier1, self).__init__()
        layers = []
        # currently 10000 units
...
class ResClassifier2(nn.Module):
    def __init__(self, num_classes= 12,num_layer =2,num_unit=152,prob=0.5,middle=76):
        super(ResClassifier2, self).__init__()
        layers = []
        # currently 10000 units
        layers.append(nn.Dropout(p=prob))
        layers.append(nn.Linear(num_unit,middle))
...
class ResClassifier3(nn.Module):
    def __init__(self, num_classes= 3,num_layer =2,num_unit=152,prob=0.5,middle=76):
        super(ResClassifier3, self).__init__()
        layers = []
        # currently 10000 units
        layers.append(nn.Dropout(p=prob))
        layers.append(nn.Linear(num_unit,middle))
